...
#define maxScale 1.3
#define minScale 0.4
#define collectionCellHeight 150.0f
...
- (void)loadUI
{
    ...
    view.frame = CGRectMake(0, 0, self.frame.size.width , 325-175);
    [self addSubview:view];
    self.bottomContentView = view;
    
    [self.bottomContentView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.bottomContentView.superview).insets(UIEdgeInsetsMake(CGRectGetHeight(self.frame) - view.frame.size.height, 0, 0, 0));
    }];
...
//    
//    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.bottomContentView.frame.size.width, 175)
...
- (void)loadData
{
    self.rowData = @[@[NSLocalizedString(@"从相册中选择一张", nil),
...
//    [self.selectedAssets addObject:self.assets[indexPath.row]];
    WeiXinPickReusableView *pictureSelectView = [self.indexPathToCheckViewTable objectForKey:indexPath];
//    pictureSelectView.cellSelected = [self.selectedAssets containsObject:self.assets[indexPath.row]];
    pictureSelectView.cellSelected = YES;
    ...
    [self dismissBlock:^(BOOL Complete) {
        if (self.clickBlock) {
            ...
            self.clickBlock(image, PickPictureSendPicture);
        }
    }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell.textLabel.text isEqual:@"取消"]) {
        cell.textLabel.textColor = RGBColor(113.0f, 175.0f, 255.0f);
    } else {
        cell.textLabel.textColor = RGBColor(51.0f, 51.0f, 51.0f);
    }
    return cell;
}
